...
| `sudo apt install package` | Install a package, or update it to the latest version |
| `sudo apt remove package` | Remove a package (use `purge` instead to also remove configuration files) |
| `sudo apt upgrade` | Upgrade all installed packages to their latest versions |
| `apt list [-i\|--installed]` | List all installed packages |
| `apt --help` | Show command usage and option reference |
| `man apt` | Open the full manual page |
| `whatis apt` | Print one-line command description |
| `type -a apt` | Show how the shell resolves the command path |
| `command -v apt` | Print executable path used by current shell |
| `tldr apt` | Show concise command examples |
| `apropos apt` | Search manual database for related commands |
| `apt --version` | Print local tool version |
