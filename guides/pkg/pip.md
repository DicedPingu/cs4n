...
| Command | Use case |
| --- | --- |
| `pip install package` | Install a package (see `pip install` for more install examples) |
| `pip install --user package` | Install a package to the user's directory instead of the system-wide default location |
| `pip install [-U\|--upgrade] package` | Upgrade a package |
| `pip uninstall package` | Uninstall a package |
| `pip freeze > requirements.txt` | Save installed packages to file |
| `pip list` | List installed packages |
| `pip show package` | Show installed package info |
| `pip install [-r\|--requirement] requirements.txt` | Install packages from a file |
| `pip --help` | Show command usage and option reference |
| `man pip` | Open the full manual page |
| `whatis pip` | Print one-line command description |
| `type -a pip` | Show how the shell resolves the command path |
| `command -v pip` | Print executable path used by current shell |
| `tldr pip` | Show concise command examples |
| `apropos pip` | Search manual database for related commands |
| `pip --version` | Print local tool version |
