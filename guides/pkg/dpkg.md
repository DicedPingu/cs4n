...
| Command | Use case |
| --- | --- |
| `sudo dpkg [-i\|--install] path/to/file.deb` | Install a package |
| `sudo dpkg [-r\|--remove] package` | Remove a package |
| `dpkg [-l\|--list] pattern` | List installed packages |
| `dpkg [-L\|--listfiles] package` | List a package's contents |
| `dpkg [-c\|--contents] path/to/file.deb` | List contents of a local package file |
| `dpkg [-S\|--search] path/to/file` | Find out which package owns a file |
| `sudo dpkg [-P\|--purge] package` | Purge an installed or already removed package, including configuration |
| `dpkg --help` | Show command usage and option reference |
| `man dpkg` | Open the full manual page |
| `whatis dpkg` | Print one-line command description |
| `type -a dpkg` | Show how the shell resolves the command path |
| `command -v dpkg` | Print executable path used by current shell |
| `tldr dpkg` | Show concise command examples |
| `apropos dpkg` | Search manual database for related commands |
| `dpkg --version` | Print local tool version |
